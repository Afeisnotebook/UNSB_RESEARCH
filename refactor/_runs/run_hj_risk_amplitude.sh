...
MET=/home/yc/unsb_tired/refactor/_runs/metrics_hj_risk
EVAL_PY=/home/yc/UNSB_Cov5/tools/evaluate_restoration.py
DIAG=/home/yc/unsb_tired/refactor/_runs/diagnostics/hj_risk_amplitude.jsonl
GPU=0
mkdir -p "$MET"
cd "$ROOT"
...
"$PY" train.py "${COMMON[@]}" --name "$NAME" --model hj --n_epochs 200 --save_epoch_freq 50 \
  ...
  --hj_enable True --hj_layers 0 --hj_start_epoch 5 --hj_schedule constant \
  ...
  --hj_control true --hj_amplitude risk --hj_diag_out "$DIAG" > "$MET/${NAME}_train.log" 2>&1
