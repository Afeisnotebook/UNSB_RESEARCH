# G1-DT-RHGC8: Receding-Horizon GAN Consensus for DT

Parent probe: `dt`.

## Evidence

`loss_delta::G_GAN` at H=8 predicts H=200: BA=0.750, Spearman=0.676, domains=4/6.

## Operator

From the same immutable full state S_k, compute B0=Phi_0^8(S_k) and Bi=Phi_dt^8(S_k). Let Delta=G_GAN(Bi)-G_GAN(B0). Commit Bi iff Delta<0; otherwise commit B0.
