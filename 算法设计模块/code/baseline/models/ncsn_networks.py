...
from .det_pad import DeterministicReflectionPad2d
import math 
...
class ResnetGenerator_ncsn(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=9,
        ...
        model = [DeterministicReflectionPad2d(3),
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                ...
                model_upsample += [
                    ...
                    norm_layer(int(ngf * mult / 2)),
                    nn.ReLU(True)]
        model_upsample += [DeterministicReflectionPad2d(3)]
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        if padding_type == 'reflect':
            conv_block += [DeterministicReflectionPad2d(1)]
...
class ResnetBlock_cond(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias,temb_dim,z_dim):
        ...
        p = 0
        if padding_type == 'reflect':
            self.conv_block += [DeterministicReflectionPad2d(1)]
        elif padding_type == 'replicate':
        ...
        p = 0
        if padding_type == 'reflect':
            self.conv_fin += [DeterministicReflectionPad2d(1)]
        elif padding_type == 'replicate':
...
def get_pad_layer(pad_type):
    if(pad_type in ['refl', 'reflect']):
        PadLayer = DeterministicReflectionPad2d
