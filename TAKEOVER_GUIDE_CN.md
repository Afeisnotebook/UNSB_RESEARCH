...
> 更新：2026-08-27。本文回答新同事最先需要知道的三件事：仓库里有什么、已经得出了什么结论、这些内容位于研究时间线的哪一段。
...
3. 旧 continuous DT/HJ 的 clean 负裁决仍成立；SEARCH-002 重新打开的是 **finite-horizon HJ → plain handoff**，不是旧配置调参。
4. finite-horizon HJ 在独立 discovery70 上 step1200 为 `+0.710548 dB`、6/6 域正；关闭 HJ 后 step1600 仍 `+0.660975 dB`，当前分类 `positive_but_fragile`。
5. HNEK `gamma=0.25` 降为递补一；DT、LTTR、DCUM/LBST/PTQ/AEB 当前实现不进入下一轮。
...
| `research/candidates/` | DT/HJ/HNEK 与新机制的身份、代码和演进 | finite-horizon HJ 为当前脆弱候选；HNEK 为递补 | T1–T3、T5、T8–T10 | [候选索引](./research/candidates/README.md) |
| `research/searches/` | 冻结的筛选/重推导/验证控制器 | SEARCH-002 本地完成，待 4090 | T8–T10→下一阶段 | [搜索索引](./research/searches/README.md) |
