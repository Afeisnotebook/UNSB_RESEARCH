...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    init_modules =[]
    init_modules.append(conv3x3(channels, nf))
    # modules.append(conv3x3(channels, nf))
    hs_c = [nf]

    in_ch = nf
    for i_level in range(self.num_downsampling):
      mult = 2**(i_level+1)
      out_ch = nf * mult
      modules.append(ResnetBlock(in_ch=in_ch, out_ch=out_ch,down=True))
      in_ch = out_ch
    ...
    for i_level in range(self.num_downsampling):
      # mult = 2**(i_level+1)
      out_ch = in_ch // 2
      modules.append(ResnetBlock(in_ch=in_ch, out_ch=out_ch,up=True))
      in_ch = out_ch
      # out_ch = nf*mult
      # Residual blocks for this resolution
#       if i_level is num_resolutions-1:
#         for i_block in range(num_res_blocks):
...
#     assert not hs_c
    fin_modules =[]
    if progressive != 'output_skip':
      fin_modules.append(nn.GroupNorm(num_groups=min(in_ch // 4, 32),
                                  num_channels=in_ch, eps=1e-6))
      fin_modules.append(conv3x3(in_ch, channels, init_scale=init_scale))
    self.embed_modules = nn.ModuleList(modules_emb)
    self.all_modules = nn.ModuleList(modules)
    self.fin_modules = nn.ModuleList(fin_modules)
    self.init_modules = nn.ModuleList(init_modules)
  ...
  def forward(self, x, time_cond, z):
    ...
    if self.conditional:
      for mod in self.embed_modules:
          # print(temb.shape)
        temb = mod(temb)
        # print(self.act(temb).shape)
          # m_idx += 1
        temb = self.act(temb)
        # temb = self.act(temb))
          # m_idx += 1
    ...
    if self.progressive_input != 'none':
      input_pyramid = x
    for mod in self.init_modules:
      h = mod(x)
    for mod in self.all_modules:
      h = mod(h, temb,zemb)
    for i,mod in enumerate(self.fin_modules):
      if i==0:
        h = mod(h)
        h = self.act(h)
      else:
