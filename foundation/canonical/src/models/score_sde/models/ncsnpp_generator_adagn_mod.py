...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    channels = config.num_channels * 2
  ...
  def forward(self, x, time_cond, z):
    ...
    z_null = torch.zeros(size=[x.shape[0],200]).cuda()
    zemb = self.z_transform(z_null)
    ...
    if not self.not_use_tanh:
        ...
        # return (x_init - h)[:,:self.config.num_channels]
        # import matplotlib.pyplot as plt
        # print(h[:,self.config.num_channels:].mean().item())
        # img0 = torch.tanh(h[:,:self.config.num_channels])
        # img0 = 0.5 * img0 + 0.5
        # plt.figure()
        # plt.imshow(img0[0].reshape(32,32).detach().cpu().numpy())
        # plt.savefig('img0.jpg')
        # img1 = torch.tanh(h[:,:self.config.num_channels]) + h[:,self.config.num_channels:] * z
        # img1 = 0.5 * img1 + 0.5
        # img1 = torch.clamp(img1, 0, 1)
        # plt.figure()
        # plt.imshow(img1[0].reshape(32,32).detach().cpu().numpy())
        # plt.savefig('img1.jpg')
        # return 0
        return h[:,:self.config.num_channels] + h[:,self.config.num_channels:] * z
